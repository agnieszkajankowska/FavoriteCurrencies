...
const mapStateToProps = state => ({
    ...
    failure: state.favoritesData.failure
});
                ...
                <h4 className="favorites-list-title">Favorites List:</h4>
                {
                    this.props.failure ?
                        <Alert bsStyle="danger">
                            There was a problem trying to get currency exchange rate. Please try again.
                        </Alert> :
                        (this.props.chosenCurrencies.length > 0 ?
                            <Button onClick={this.handleDeleteAll} bsStyle="danger" className="delete-all-btn">Delete
                                All</Button> :
                            <Alert bsStyle="info">
                                <strong>Hey there!</strong> Here, you will see currencies exchange rate, after adding to
                                favorites.
                            </Alert>)
                }
                <PanelGroup
